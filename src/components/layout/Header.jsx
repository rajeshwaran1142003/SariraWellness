...
import { SariraLogo } from '@/components/icons/Icons';
    ...
    <header className={cn(
      ...
      <div className="container mx-auto px-4 flex justify-between items-center py-2">
        <Link href="/" className="text-primary">
          <SariraLogo className="h-14" height={56} width={168} />
        </Link>
        <nav className="hidden md:flex space-x-6 items-center">
          ...
          <Button asChild>
            <Link href={SITE_CONFIG.contact.whatsapp} target="_blank">Order Now</Link>
          </Button>
        </nav>
        ...
        <nav className="flex flex-col items-center justify-center h-[calc(100%-80px)] space-y-8">
          {NAVIGATION.main.map(link => (
           ...
           <Button asChild size="lg">
            <Link href={SITE_CONFIG.contact.whatsapp} target="_blank" onClick={closeMenu}>Order Now</Link>
          </Button>
        </nav>
      </div>
    </header>
