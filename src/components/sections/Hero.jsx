...
import { ShieldCheck, HeartPulse, Baby, Sparkles } from '@/components/icons/Icons';
import { cn } from '@/lib/utils';
import { HERO_CONTENT, SITE_CONFIG } from '@/config/content';
...
export function Hero() {
  ...
  useEffect(() => {
    if (isMounted) {
      ...
      const type = () => {
        const currentHeadline = HERO_CONTENT.headlines[headlineIndex];
        ...
        if (isDeleting) {
          if (charIndex > 0) {
            ...
            isDeleting = false;
            setHeadlineIndex((prevIndex) => (prevIndex + 1) % HERO_CONTENT.headlines.length);
          }
        } else {
    ...
    <section id="home" className="relative pt-32 pb-20 md:pt-48 md:pb-28 overflow-hidden bg-gradient-to-b from-background to-secondary">
      ...
      <div className="container mx-auto px-4 flex flex-col lg:flex-row items-center justify-center relative z-10 gap-12">
        <div className="w-full text-center">
            <p className="text-4xl md:text-5xl lg:text-6xl font-headline text-primary mb-4">{HERO_CONTENT.welcomeText}</p>
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button asChild size="lg" className="bg-accent text-accent-foreground hover:bg-accent/90">
              <Link href="/product">{HERO_CONTENT.ctaButtons.primary}</Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="border-accent text-accent hover:bg-accent hover:text-accent-foreground">
              <Link href="/tamil-culture">{HERO_CONTENT.ctaButtons.secondary}</Link>
            </Button>
          </div>
          <div className="mt-12 max-w-3xl mx-auto">
           ...
           <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                {HERO_CONTENT.features.map(feature => {
                  ...
                  return (
                    <div key={feature.text} className="flex flex-col items-center gap-3">
                        <div className="text-accent">
                          {IconComponent && <IconComponent className="w-8 h-8" />}
                        </div>
                        <span className="text-base font-medium text-primary">{feature.text}</span>
                    </div>
                  );
                })}
            </div>
